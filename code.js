...
function extractPadding(node) {
    ...
    if ('paddingTop' in node) {
        return {
            top: (_a = node.paddingTop) !== null && _a !== void 0 ? _a : 0,
            right: (_b = node.paddingRight) !== null && _b !== void 0 ? _b : 0,
            bottom: (_c = node.paddingBottom) !== null && _c !== void 0 ? _c : 0,
            left: (_d = node.paddingLeft) !== null && _d !== void 0 ? _d : 0
        };
    }
    return undefined;
}
...
function hasVisibleContent(node) {
    ...
    if ('fills' in node || 'strokes' in node) {
        const hasFills = 'fills' in node && Array.isArray(node.fills) && node.fills.length > 0 && node.fills.some((f) => f.visible !== false);
        const hasStrokes = 'strokes' in node && Array.isArray(node.strokes) && node.strokes.length > 0 && node.strokes.some((s) => s.visible !== false);
        return hasFills || hasStrokes;
    }
    return true;
}
// Export node as SVG
async function exportAsSvg(node) {
    try {
...
async function extractBlockStructure(node, depth = 0, onProgress) {
...
function formatForConsole(structure, indent = 0) {
    ...
    if (structure.fills && structure.fills !== figma.mixed && Array.isArray(structure.fills)) {
        const solidFill = structure.fills.find((f) => f.type === 'SOLID' && f.visible !== false);
        if (solidFill) {
    ...
    if (structure.strokes && Array.isArray(structure.strokes) && structure.strokes.length > 0) {
        const solidStroke = structure.strokes.find((s) => s.type === 'SOLID' && s.visible !== false);
        if (solidStroke) {
...
function fillsToBackground(fills) {
    ...
    if (solidFill) {
        const { r, g, b } = solidFill.color;
        const a = (_a = solidFill.opacity) !== null && _a !== void 0 ? _a : 1;
        const toHex = (n) => Math.round(n * 255).toString(16).padStart(2, '0');
        if (a < 1) {
    ...
    const gradientFill = fills.find((f) => (f.type === 'GRADIENT_LINEAR' || f.type === 'GRADIENT_RADIAL' || f.type === 'GRADIENT_ANGULAR' || f.type === 'GRADIENT_DIAMOND') && f.visible !== false);
    if (gradientFill) {
        const stops = gradientFill.gradientStops.map(stop => {
            var _a;
            const { r, g, b } = stop.color;
            const a = (_a = stop.color.a) !== null && _a !== void 0 ? _a : 1;
            const toHex = (n) => Math.round(n * 255).toString(16).padStart(2, '0');
            const color = a < 1
...
function generateCleanStructure(structure) {
    var _a;
    const clean = {
        name: structure.name,
        type: structure.type,
    ...
    if (structure.strokes && Array.isArray(structure.strokes) && structure.strokes.length > 0) {
        ...
        if (solidStroke) {
            const { r, g, b } = solidStroke.color;
            const a = (_a = solidStroke.opacity) !== null && _a !== void 0 ? _a : 1;
            const toHex = (n) => Math.round(n * 255).toString(16).padStart(2, '0');
...
function generateTailwindClasses(structure) {
    const classes = [];
    ...
    if (structure.padding) {
        ...
        const getPaddingValue = (val) => paddingMap[val] || `[${val}px]`;
        if (top === right && right === bottom && bottom === left && top > 0) {
            ...
            if (top === bottom && left === right && top > 0 && left > 0) {
                ...
                if (top > 0)
                    classes.push(`pt-${getPaddingValue(top)}`);
                if (right > 0)
                    classes.push(`pr-${getPaddingValue(right)}`);
                if (bottom > 0)
                    classes.push(`pb-${getPaddingValue(bottom)}`);
                if (left > 0)
                    classes.push(`pl-${getPaddingValue(left)}`);
            }
        }
    }
    // Layout mode (Flexbox)
    ...
    if (structure.cornerRadius !== undefined && structure.cornerRadius !== 0 && structure.cornerRadius !== figma.mixed) {
        const radius = structure.cornerRadius;
        const radiusMap = {
    ...
    if (structure.opacity !== undefined && structure.opacity !== 1) {
        ...
        const closestOpacity = standardOpacities.reduce((prev, curr) => Math.abs(curr - opacityPercent) < Math.abs(prev - opacityPercent) ? curr : prev);
...
function generateTailwindOutput(structure, indent = 0) {
    const prefix = '  '.repeat(indent);
    let output = '';
